...
// ── MACHINES ──────────────────────────────────────────────────────────────────
router.get('/zones/:zoneId/machines', requireAuth, (req, res) => {
...
// ── MEASUREMENTS ──────────────────────────────────────────────────────────────
router.get('/components/:compId/measurements', requireAuth, (req, res) => {
  const ms = Q.getMeasurementsByComponent.all(req.params.compId);
  res.json(ms.map(m => ({
    ...m,
    images: m.image_files ? m.image_files.split(',').map(f => `/uploads/${f}`) : []
  })));
});

router.get('/measurements/alerts', requireAuth, (req, res) => {
...
router.post('/components/:compId/measurements', requireAdmin, upload.array('images', 10), (req, res) => {
  ...
  if (req.files && req.files.length) {
    req.files.forEach(f => Q.insertImage.run(id, f.filename, f.originalname, f.mimetype));
  }
...
// ── PDF REPORT ────────────────────────────────────────────────────────────────
router.get('/measurements/:id/pdf', requireAuth, (req, res) => {
...
router.get('/machines/:id/pdf', requireAuth, (req, res) => {
  const PDFDocument = require('pdfkit');
