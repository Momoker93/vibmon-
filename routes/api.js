...
router.post('/zones/:zoneId/machines', requireAdmin, async (req, res) => {
  try {
    ...
    if (!name) return res.status(400).json({ error: 'Nombre obligatorio' });
    const id = uid();
    await Q.insertMachine(id, req.params.zoneId, name, type || '', rpm || null, notes || '', req.body.icon || '⚙');
    const comps = [];
...
router.put('/machines/:id', requireAdmin, async (req, res) => {
  try {
    const { name, type, rpm, notes, components } = req.body;
    await Q.updateMachine(name, type || '', rpm || null, notes || '', req.params.id, req.body.icon || '⚙');
    if (components) {
